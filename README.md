...
**Store 管存储，System 管运行，Mutation 管编排，Context 管协调，View 管表现。**
...
- 通过 `Query<T>()` 读取 Store 数据，通过 `Mutate<T>()` 触发 Mutation
...
public class DungeonSystem : GameSystemBase
{
    ...
    // 通过 Mutation 将结果写入持久数据
    private async UniTaskVoid SettleDungeonResult()
    {
        var result = await Mutate(new DungeonSettleMutation(...));
    }
}
```
...
Mutation 是**业务操作的唯一编排入口**。当一个操作涉及多个 Store、或同时涉及 Store 和 System 时，由 Mutation 统一编排，确保业务逻辑集中在一处，调用方只需关心结果。

数据和执行逻辑在同一个 `readonly struct` 中，无需单独的 Handler 类。

**使用原则：**
> Store 的公开方法是 Mutation 调用的原子构件。
> System 的公开方法是 Mutation 驱动运行时的手段。
> 调用方（UI/场景）只调 `Mutate()`，不需要知道内部编排细节。

**关键特征：**
- Mutation 是 `readonly struct`，同时携带数据和 `Mutate` 方法
- 通过 `context.Query<T>()` 获取只读查询接口做前置检查
- 通过 `context.GetStore<T>()` 获取具体 Store 进行持久数据变更
- 通过 `context.GetSystem<T>()` 获取 System 驱动运行时逻辑
- Mutate 返回 `MutationResult`（同步），前置检查合并在 Mutate 开头
- **无需注册**：`GameContext.Mutate()` 直接调用 `mutation.Mutate(this)`

**示例（纯 Store 编排）：**

```csharp
public readonly struct PurchaseItemMutation : IMutation
{
    public readonly int ItemId;

    public PurchaseItemMutation(int itemId) => ItemId = itemId;

    public MutationResult Mutate(IMutationContext ctx)
    {
        var currency = ctx.Query<ICurrencyQueries>();
        if (!currency.HasEnough(CurrencyType.Gold, GetPrice(ItemId)))
            return MutationResult.Fail("金币不足");
...
```csharp
public readonly struct DungeonEnterMutation : IMutation
{
    public readonly int DungeonId;

    public DungeonEnterMutation(int dungeonId) => DungeonId = dungeonId;

    public MutationResult Mutate(IMutationContext ctx)
    {
        var dungeon = ctx.Query<IDungeonQueries>();
        if (!dungeon.IsDungeonUnlocked(DungeonId))
            return MutationResult.Fail("副本未解锁");
...
GameContext 是上层架构的核心枢纽，管理所有 Store 和 System 的注册与生命周期，并作为 Mutation 的集中分派点。

**核心 API：**

| 接口 | API | 用途 | 典型调用方 |
|------|-----|------|-----------|
| IGameContext | `Query<T>()` | 获取 Store 只读查询接口 | UI、System、Mutation、任意代码 |
| IGameContext | `GetSystem<T>()` | 获取 System 实例 | Mutation、System 互访、场景控制器 |
| IGameContext | `Mutate<T>(mutation)` | 执行 Mutation | UI、System |
| IMutationContext | `GetStore<T>()` | 获取具体 Store 实例（写权限） | Mutation.Mutate |
...
两者均提供与 `GameSystemBase` 一致的快捷方法：`Query<T>()`、`GetSystem<T>()`、`Mutate<T>()`。
`GameUIView` 额外提供 `ExecuteMutation<T>()`：执行 Mutation + 失败时自动弹出错误提示。
...
public class UIDungeonSelectWindow : GameUIView
{
    ...
    {
        ExecuteMutation(new DungeonEnterMutation(1)).Forget();
        CloseWindow();
    }
}
```
...
// ✅ 在 UI 面板中执行 Mutation（推荐）
public class MyPanel : GameUIView
{
    private void OnBuyClick()
    {
        ExecuteMutation(new PurchaseItemMutation(itemId));
    }
}

// ✅ 在 Mutation.Mutate 中编排 Store 和 System
public MutationResult Mutate(IMutationContext ctx)
{
...
// ✅ 在非 View 代码中通过能力接口访问
var result = this.Mutate(new PurchaseItemMutation { ... });
```
...
涉及多 Store 或 Store + System      → 走 Mutation（业务编排的唯一入口）
```
...
1. 定义 Mutation 并实现 Mutate：

```csharp
public readonly struct MyMutation : IMutation
{
    public readonly int Param;

    public MyMutation(int param) => Param = param;
...
无需注册，直接可用：`Mutate(new MyMutation(42));`
...
3. 使用 `ExecuteMutation()` 执行带错误提示的 Mutation
...
│   ├── Mutation/
│   │   ├── IMutation.cs            Mutation 接口（自执行，含 Mutate 方法）
│   │   └── MutationResult.cs       Mutation 结果（IsSuccess + Message）
│   ├── Context/
...
| 泛型参数 | T + PascalCase | `TData`, `TMutation` |
...
// readonly struct（Mutation / Event）
public readonly struct NewGameMutation : IMutation
{
